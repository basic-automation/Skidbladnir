...
pub fn run() {
	tauri::Builder::default().invoke_handler(tauri::generate_handler![commands::encoder_version, commands::default_settings, commands::validate_settings, commands::convert_image]).run(tauri::generate_context!()).expect("Skidbladnir failed to start");
}
