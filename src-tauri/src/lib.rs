...
pub fn run() {
	tauri::Builder::default().plugin(tauri_plugin_dialog::init()).manage(commands::CancelFlag::default()).invoke_handler(tauri::generate_handler![commands::encoder_version, commands::default_settings, commands::validate_settings, commands::convert_image, commands::inspect_dropped_paths, commands::load_preferences, commands::save_preferences, commands::cancel_conversion]).run(tauri::generate_context!()).expect("Skidbladnir failed to start");
}
